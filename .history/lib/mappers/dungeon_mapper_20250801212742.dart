...
class DungeonMapper {
  ...
  static Dungeon fromDto(DungeonGenerationDto dto) {
    return Dungeon(
      type: dto.type.description,
      builderOrInhabitant: dto.builderOrInhabitant.description,
      status: dto.status.description,
      objective: dto.objective.description,
      target: dto.target.description,
      targetStatus: dto.targetStatus.description,
      location: dto.location.description,
      entry: dto.entry.description,
      sizeFormula: dto.sizeFormula,
      occupantI: dto.occupantI,
      occupantII: dto.occupantII,
      leader: dto.leader,
      rumorSubject: dto.rumorSubject.description,
      rumorAction: dto.rumorAction.description,
      rumorLocation: dto.rumorLocation.description,
      fullObjective: dto.fullObjective,
      fullRumor: dto.fullRumor,
    );
  }
  ...
  static DungeonGenerationDto toDto(Dungeon dungeon) {
    ...
    final builderOrInhabitant = _findDungeonBuilder(dungeon.builderOrInhabitant);
    final status = _findDungeonStatus(dungeon.status);
    final objective = _findDungeonObjective(dungeon.objective);
    final target = _findDungeonTarget(dungeon.target);
    final targetStatus = _findDungeonTargetStatus(dungeon.targetStatus);
    final location = _findDungeonLocation(dungeon.location);
    final entry = _findDungeonEntry(dungeon.entry);
    final rumorSubject = _findRumorSubject(dungeon.rumorSubject);
    final rumorAction = _findRumorAction(dungeon.rumorAction);
    final rumorLocation = _findRumorLocation(dungeon.rumorLocation);
    ...
    return DungeonGenerationDto(
      ...
      location: location,
      entry: entry,
      sizeFormula: dungeon.sizeFormula,
      occupantI: dungeon.occupantI,
      occupantII: dungeon.occupantII,
