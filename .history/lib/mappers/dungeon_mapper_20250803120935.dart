...
class DungeonMapper {
  ...
  static Room fromRoomDto(
    ...
    if (useTreasureByLevel && treasureLevel != null) {
      ...
      specialItem = components['specialItem'] ?? '';
      magicItem = components['magicItem'] ?? '';
      specialTreasure = ''; // Não usa specialTreasure quando é por nível
    } else {
      // Usa tesouro da tabela
  ...
  /// Extrai os componentes do tesouro por nível
  static Map<String, String> _extractTreasureComponents(String treasureByLevel) {
    ...
    for (final line in lines) {
      if (line.contains('PO:') || line.contains('PP:')) {
        ...
        // Remove "Nenhuma Gema" ou "Nenhum Objeto de Valor"
        if (!line.contains('Nenhuma Gema') && !line.contains('Nenhum Objeto de Valor')) {
          specialItem += (specialItem.isNotEmpty ? ' + ' : '') + line;
        }
    ...
    components['specialItem'] = specialItem;
