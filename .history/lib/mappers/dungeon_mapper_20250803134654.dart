...
class DungeonMapper {
  ...
  static Room fromRoomDto(
    ...
    if (useTreasureByLevel) {
      // Usa tesouro por nível - coloca tudo no campo treasure
      final levelToUse = treasureLevel ?? TreasureLevel.level1; // Usa nível 1 como padrão
      final treasureByLevel = TreasureResolver.resolveByLevel(levelToUse);
      treasure = treasureByLevel;
      specialTreasure = ''; // Limpa specialTreasure quando usa tesouro por nível
      magicItem = ''; // Limpa magicItem quando usa tesouro por nível
  ...
  static Map<String, String> _extractTreasureComponents(
    ...
    for (final line in lines) {
      if (line.contains('PO:') || line.contains('PP:')) {
        ...
        // Sempre inclui itens mágicos, mesmo que seja "Nenhum Item Mágico"
