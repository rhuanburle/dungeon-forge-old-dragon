  ...
  void _showGenerationDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Gerar Nova Masmorra'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text('Quantidade de salas (opcional):'),
            const SizedBox(height: 8),
            TextField(
              controller: _roomCountController,
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(
                hintText: 'Deixe vazio para usar valor da tabela',
                border: OutlineInputBorder(),
              ),
            ),
            const SizedBox(height: 16),
            const Text(
              'Dica: Deixe vazio para usar o valor da Tabela 9-1 (coluna 9)',
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              final input = _roomCountController.text.trim();
              _customRoomCount = input.isEmpty ? 0 : int.tryParse(input) ?? 0;
