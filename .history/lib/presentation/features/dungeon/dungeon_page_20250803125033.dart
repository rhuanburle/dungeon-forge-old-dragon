...
class _DungeonPageState extends State<DungeonPage> {
  ...
  TreasureLevel _selectedTreasureLevel =
      TreasureLevel.level1; // Default to level 1
  bool _useTreasureByLevel = false;

  /// Sincroniza automaticamente o TreasureLevel baseado no PartyLevel
  void _syncTreasureWithPartyLevel(PartyLevel partyLevel) {
  ...
  void _regenerateOccupants() {
    if (!_useEncounterTables || _dungeon == null) return;

    // Usa a mesma instância do gerador para garantir que os dados sejam compartilhados
    final generator = DungeonGeneratorRefactored();
    
    // Primeiro, regenera os dados da masmorra com as novas configurações
    ...
    // Depois, atualiza apenas os ocupantes das salas existentes
    generator.updateRoomOccupants(_dungeon!);
    setState(() {});
  }
    ...
    return DropdownButtonFormField<PartyLevel>(
      ...
      onChanged: (value) {
        setState(() {
          ...
          _syncTreasureWithPartyLevel(_selectedPartyLevel);
        });
      },
    );
  }
          ...
          SwitchListTile(
            ...
            onChanged: (bool value) {
              setState(() {
                ...
                // Sincroniza tesouro quando ativa encontros
                if (value) {
                  _syncTreasureWithPartyLevel(_selectedPartyLevel);
                }
              });
