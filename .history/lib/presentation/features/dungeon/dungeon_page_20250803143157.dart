...
import '../../shared/widgets/navigable_room_card.dart';
...
class _DungeonPageState extends State<DungeonPage> {
  ...
  // Estado de navegação das salas
  int _currentRoomIndex = 1; // Sala atual (padrão: sala 1)
  Set<int> _visitedRooms = <int>{}; // Salas visitadas
  ...
  void _generate() {
    ...
    // Resetar navegação quando gerar nova masmorra
    _currentRoomIndex = 1;
    _visitedRooms.clear();
    _visitedRooms.add(1); // Marcar sala 1 como visitada por padrão

    setState(() {});
  }

  /// Marca uma sala como atual
  void _setCurrentRoom(int roomIndex) {
    if (roomIndex >= 1 && roomIndex <= _dungeon.roomsCount) {
      setState(() {
        _currentRoomIndex = roomIndex;
        _visitedRooms.add(roomIndex); // Adiciona à lista de visitadas
      });
    }
  }

  /// Marca uma sala como visitada
  void _markRoomAsVisited(int roomIndex) {
    if (roomIndex >= 1 && roomIndex <= _dungeon.roomsCount) {
      setState(() {
  ...
  /// Limpa todas as marcações de navegação
              ...
              child: Row(
                children: [
                  ...
                  IconButton(
                    ...
                    tooltip: 'Limpar Navegação das Salas',
    ...
    return SingleChildScrollView(
      child: Column(
        children: [
          ...
          const SizedBox(height: 16),
          _buildEncounterSettingsCard(),_buildNavigationProgressCard(),
  ...
  Widget _buildNavigationProgressCard() {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: AppColors.surfaceLight,
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.navigation, color: AppColors.primaryLight, size: 20),
              const SizedBox(width: 8),
              const Text(
          ...
          const SizedBox(height: 8),
          SelectableText(
            'Salas Visitadas: ${_visitedRooms.length}/${_dungeon.roomsCount}',
            style: TextStyle(
              fontSize: 14,
              color: AppColors.primaryLight,
              fontWeight: FontWeight.w500,
          ...
          const SizedBox(height: 8),
          SelectableText(
            'Total de Salas: ${_dungeon.roomsCount}',
            style: TextStyle(
              fontSize: 14,
              color: AppColors.primaryLight,
              fontWeight: FontWeight.w500,
  ...
  Widget _buildRoomCard(Room room) {
    ...
    final isCurrentRoom = room.index == _currentRoomIndex;
    final isVisited = _visitedRooms.contains(room.index);

    return NavigableRoomCard(
      room: room,
      isCurrentRoom: isCurrentRoom,
      isVisited: isVisited,
      isEntrance: isEntrance,
      isBoss: isBoss,
      onRoomTap: () => _showRoomDetails(room),
      onNavigationTap: () => _setCurrentRoom(room.index),
    );
  }
}