...
class _ErmosPageState extends State<ErmosPage> {
    ...
    return Column(
      ...
      children: [
        ...
        _buildInfoCard('Características da Civilização', Icons.architecture, [
          _buildDetailText(_civilization!.characteristics),
        ]),
        const SizedBox(height: 16),
        // Card de Atitude e Temas
        _buildInfoCard('Atitude e Temas', Icons.people, [
          _buildDetailText(_civilization!.attitudeAndThemes),
        ]),
      ],
    );
  }

  String _getCivilizationCharacteristics(Civilization civilization) {
    final details = civilization.details;
    final lines = details.split('\n');
    final characteristics = <String>[];

    for (final line in lines) {
      if (line.contains('Características da Civilização:') ||
          line.contains('• Nível Tecnológico:') ||
          line.contains('• Aparência:') ||
          line.contains('• Alinhamento do Governo:') ||
          line.contains('• Governante:') ||
          line.contains('• Raça Dominante:') ||
          line.contains('• Especial:') ||
          line.contains('Tabelas Utilizadas:')) {
  ...
  String _getCivilizationAttitudeAndThemes(Civilization civilization) {
    final details = civilization.details;
    final lines = details.split('\n');
    final attitudeAndThemes = <String>[];

    for (final line in lines) {
      if (line.contains('Atitude e Temas:') ||
          line.contains('• Atitude com Visitantes:') ||
          line.contains('• Tema (Povoado/Aldeia/Vila):') ||
          line.contains('• Tema (Cidade/Metrópole):') ||
          line.contains('Tabela 4.42:')) {
        attitudeAndThemes.add(line);
      }
    }

    return attitudeAndThemes.join('\n');
  }
