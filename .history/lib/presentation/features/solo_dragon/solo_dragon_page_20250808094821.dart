...
class _SoloDragonPageState extends State<SoloDragonPage> {
  ...
  Widget _buildBoard() {
    ...
    TextStyle cell(bool eliminated) => TextStyle(
          ...
          decoration:
              eliminated ? TextDecoration.lineThrough : TextDecoration.none,
        );
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        for (int i = 0; i < _rumors.length; i++)
          Padding(
            ...
            child: Row(
              children: [
                Expanded(
                  child: Text(
                    _rumors[i].createdBy,
                    style: cell(board.eliminatedA[i]),
                  ),
                ),
                ...
                Expanded(
                  child: Text(
                    _rumors[i].purpose,
                    style: cell(board.eliminatedB[i]),
                  ),
                ),
                const SizedBox(width: 8),
                Expanded(
                  child: Text(
                    _rumors[i].target,
                    style: cell(board.eliminatedC[i]),
                  ),
                ),
              ],
            ),
        ...
        if (board.hasDiscoveryA)
          Text('Descoberta A: ${board.remainingA}',
              style: const TextStyle(color: Colors.greenAccent)),
        if (board.hasDiscoveryB)
          Text('Descoberta B: ${board.remainingB}',
              style: const TextStyle(color: Colors.greenAccent)),
        if (board.hasDiscoveryC)
          Text('Descoberta C: ${board.remainingC}',
              style: const TextStyle(color: Colors.greenAccent)),
        if (board.isFinalMysteryRevealed)
          const Text('Mistério desvendado! Vá para a Câmara Final.',
