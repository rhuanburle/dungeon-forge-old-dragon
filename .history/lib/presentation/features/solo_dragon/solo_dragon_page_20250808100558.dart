...
class _SoloDragonPageState extends State<SoloDragonPage> {
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: _buildInfoItem('Tipo', _setup!.type),
              ),
              Expanded(
                child: _buildInfoItem('Entrada', _setup!.entrance),
              ),
              Expanded(
      ...
      child: Column(
        ...
        children: [
          ...
          if (board.hasDiscoveryA)
            _buildDiscovery('A', board.remainingA!),
          if (board.hasDiscoveryB)
            _buildDiscovery('B', board.remainingB!),
          if (board.hasDiscoveryC)
            _buildDiscovery('C', board.remainingC!),
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              _buildRoomDetail('Tipo', room.type),
              _buildRoomDetail('Portas', room.doors),
              _buildRoomDetail('Conteúdo', room.content),
              if (room.trap.isNotEmpty) _buildRoomDetail('Armadilha', room.trap),
              if (room.encounter.isNotEmpty) _buildRoomDetail('Encontro', room.encounter),
              if (room.treasure.isNotEmpty) _buildRoomDetail('Tesouro', room.treasure),
            ],
          ),
        ),
        const SizedBox(height: 12),
    ...
    if (_service.rollInvestigationFound()) {
      ...
      final column = colRoll <= 2
          ...
          : colRoll <= 4
              ? ColumnId.b
              : ColumnId.c;
      final rowIdx = rowRoll <= 2 ? 0 : rowRoll <= 4 ? 1 : 2;
  ...
  void _generateNextRoom() {
    // TODO: Implementar geração de salas usando A5.4, A5.5, A5.6
    final newRoom = SoloRoom(
      type: 'Câmara Padrão',
