...
class _TurnMonitorPageState extends State<TurnMonitorPage> {
  ...
  void _addTurnWithAction(DungeonAction action) {
    _service.addTurn(
      isExploration: action == DungeonAction.move || 
                    action == DungeonAction.search ||
                    action == DungeonAction.overcomeObstacle,
      isFighting: action == DungeonAction.combat,
  ...
  void _addRandomEncounter() {
    if (_service.checkForEncounter()) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Encontro gerado: ${encounter.creature} (Roll: ${encounter.roll})'),
  ...
  Widget _buildGroupDataDisplay() {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Nome: ${groupData.groupName.isEmpty ? "Não definido" : groupData.groupName}'),
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: DungeonAction.values.map((action) {
                return ActionButton(
                  onPressed: () => _addTurnWithAction(action),
  ...
  IconData _getActionIcon(DungeonAction action) {
    switch (action) {
      case DungeonAction.move:
        return Icons.directions_walk;
      case DungeonAction.talk:
        return Icons.chat;
      case DungeonAction.search:
        return Icons.search;
      case DungeonAction.overcomeObstacle:
        return Icons.terrain;
      case DungeonAction.stayAlert:
        return Icons.visibility;
      case DungeonAction.castRitual:
        return Icons.auto_fix_high;
      case DungeonAction.combat:
        return Icons.gps_fixed;
      case DungeonAction.helpEachOther:
        return Icons.people;
      case DungeonAction.other:
