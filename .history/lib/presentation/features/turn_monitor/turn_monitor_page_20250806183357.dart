...
class _TurnMonitorPageState extends State<TurnMonitorPage> {
  ...
  void _addTurnWithAction(ActionType action) {
    switch (action) {
      ...
      case ActionType.moveExploration:
        _service.addMovementAction(
          ...
          action: TurnAction(
            type: action,
            description: action.description,
            movementDistance: _service.calculateMovement(
              MovementType.exploration,
            ),
            isExploration: true,
          ),
        );
      ...
      case ActionType.rest:
        _service.addRestAction();
        break;
      case ActionType.search:
        _service.addSearchAction(searchType: action);
        break;
      case ActionType.searchSecret:
        _service.addSearchAction(searchType: action);
        break;
      case ActionType.searchTraps:
        _service.addTrapSearchAction();
        break;
      case ActionType.combat:
        _service.addTurn(
          isFighting: true,
          notes: action.description,
          action: TurnAction(type: action, description: action.description),
        );
        break;
    ...
    // Verifica se é hora de rolar encontro
    if (_service.shouldRollEncounter()) {
      _showEncounterRollDialog();
    }
  }

  void _showEncounterRollDialog() {
    final hasLight =
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
              'É hora de verificar se há um encontro aleatório!',
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 12),
            Text('Turno atual: ${_service.currentTurn - 1}'),
            if (hasLight) const Text('⚠️ Luz presente - chance aumentada'),
            if (hasNoise) const Text('⚠️ Barulho presente - chance aumentada'),
            const SizedBox(height: 12),
            const Text('Chance base: 1 em 1d6', style: TextStyle(fontSize: 14)),
            if (hasLight || hasNoise)
              const Text(
                'Chance com luz/barulho: 1-2 em 1d6',
  ...
  void _rollEncounterCheck() {
    final hasLight =
        _service.groupData.hasTorch || _service.groupData.hasLantern;
    final hasNoise = false;
    ...
    if (_service.checkForEncounter(hasLight: hasLight, hasNoise: hasNoise)) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '🎲 ENCONTRO! ${encounter.creature} (Roll: ${encounter.roll}) - Distância: ${encounter.distance}m',
          ),
  ...
  void _testDoorAction(DoorType doorType) {
    ...
    switch (doorType) {
      case DoorType.locked:
        ...
        if (result.success) {
          // Se sucesso, adiciona a ação de arrombar
          _service.addLockedDoorAction(
            hasThiefTools: true,
            isThief: false,
            result: result,
          );
        } else {
          // Se falha, apenas adiciona 1 turno
          _service.addTurn(
            isDoorAction: true,
            notes: description,
            result: result,
  ...
  void _testTrapAction(TrapType trapType, bool isSearching) {
    ...
    if (isSearching) {
      ...
      // Procurar armadilhas consome 1 turno inteiro
      _service.addTrapSearchAction(result: result);
    } else {
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              if (_hasTorch)
                                Text(
                                  'Turnos restantes: $_torchTurns',
                                  style: TextStyle(
                                    color: _torchTurns <= 2
                                        ? Colors.red
                                        : Colors.white,
                                    fontWeight: _torchTurns <= 2
                                        ? FontWeight.bold
                                        : FontWeight.normal,
                                  ),
                                ),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              if (_hasLantern)
                                Text(
                                  'Turnos restantes: $_lanternTurns',
                                  style: TextStyle(
                                    color: _lanternTurns <= 6
                                        ? Colors.red
                                        : Colors.white,
                                    fontWeight: _lanternTurns <= 6
                                        ? FontWeight.bold
                                        : FontWeight.normal,
                                  ),
                                ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Próximo turno: ${_service.currentTurn}'),
                    if (_service.shouldRollEncounter())
                      Container(
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              ActionButton(
                onPressed: () => _testTrapAction(TrapType.dart, false),
