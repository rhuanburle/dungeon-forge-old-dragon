...
class DungeonGenerator {
  ...
  Dungeon generate({
    ...
    final rooms = <Room>[];
    print('Dungeon occupants - occupantI: $col10, occupantII: $col11');
  ...
  Room _generateRoom(int index, int level,
    ...
    var monsterResult = _roomCol10[DiceRoller.roll(2, 6) - 2];
    ...
    // Processa os monstros separadamente
    String monster1 = '';
    String monster2 = '';

    // Debug: mostra o resultado original da tabela
    print('Room $index - Original monster result: $monsterResult');

    // Substitui ocupantes pelos valores reais da masmorra
    // IMPORTANTE: Substituir Ocupante II primeiro para evitar conflito com Ocupante I
    if (occupantII != null) {
    ...
    // Debug: mostra o resultado após substituição
    print('Room $index - After replacement: $monsterResult');

    // Separa os monstros baseado no resultado
    if (monsterResult.contains(' + ')) {
      final parts = monsterResult.split(' + ');
      if (parts.length >= 2) {
        monster1 = parts[0].trim();
        monster2 = parts[1].trim();
        print('Room $index - Split into: monster1="$monster1", monster2="$monster2"');
      } else {
        // Caso inesperado, coloca tudo no monster1
        monster1 = monsterResult;
        monster2 = '';
        print('Room $index - Unexpected split result, using single monster: "$monster1"');
      }
    } else {
      monster1 = monsterResult;
      monster2 = ''; // Campo vazio quando há apenas um monstro
      print('Room $index - Single monster: monster1="$monster1", monster2=""');
    }
