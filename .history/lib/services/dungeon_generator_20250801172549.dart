...
class DungeonGenerator {
  ...
  Dungeon generate({
    ...
    final coreEntry = DungeonCoreTable.entries[DiceRoller.roll(2, 6) - 2];
    final locationEntry = DungeonLocationTable.entries[DiceRoller.roll(2, 6) - 2];
    final sizeEntry = RoomSizeTable.entries[DiceRoller.roll(2, 6) - 2];
    final occupantEntry = OccupantTable.entries[DiceRoller.roll(2, 6) - 2];
    final rumorEntry = RumorTable.entries[DiceRoller.roll(2, 6) - 2];
    ...
    for (int i = 1; i <= roomsCount; i++) {
      rooms.add(_generateRoom(i, level, occupantI: occupantEntry.occupant1, occupantII: occupantEntry.occupant2));
    }

    return Dungeon(
      type: coreEntry.type,
      builderOrInhabitant: coreEntry.builder,
      status: coreEntry.status,
      objective: coreEntry.fullObjective,
      location: locationEntry.location,
      entry: locationEntry.entry,
      roomsCount: roomsCount,
      occupant1: occupantEntry.occupant1,
      occupant2: occupantEntry.occupant2,
      leader: occupantEntry.leader,
      rumor1: rumorEntry.buildRumor(occupantEntry.occupant1, occupantEntry.occupant2, occupantEntry.leader),
      rooms: rooms,
    );
  }
