...
class DungeonGeneratorRefactored {
  ...
  List<Room> _generateRooms(
    ...
    for (int i = 1; i <= roomsCount; i++) {
      final roomData = _generateRoomData(level, occupantI, occupantII);
      final room = DungeonMapper.fromRoomDto(roomData, i);
      rooms.add(room);
    }
  ...
  SpecialItem? _getSpecialItem(int col5Roll, int col6Roll) {
    if (col5Roll != 9) return null; // Só se coluna 5 indicar
  ...
  MagicItem? _getMagicItem(Treasure treasure, SpecialTreasure? specialTreasure, int col15Roll) {
