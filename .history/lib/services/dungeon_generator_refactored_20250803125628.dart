...
class DungeonGeneratorRefactored {
  ...
  void updateRoomOccupants(Dungeon dungeon) {
    ...
    if (dungeonData == null) {
      print('DEBUG: dungeonData é null');
      return;
    }

    print('DEBUG: Atualizando ${dungeon.rooms.length} salas');
    print('DEBUG: occupantI: ${dungeonData.occupantI}');
    print('DEBUG: occupantII: ${dungeonData.occupantII}');
    print('DEBUG: leader: ${dungeonData.leader}');

    for (int i = 0; i < dungeon.rooms.length; i++) {
      final room = dungeon.rooms[i];
      final roomDto = _roomGenerationService.generateRoom(dungeonData, i);

      // Atualiza apenas os campos de ocupantes
      final updatedRoom = DungeonMapper.fromRoomDto(
        roomDto,
        i + 1,
        dungeonData,
        treasureLevel: null, // Mantém o tesouro atual
        useTreasureByLevel: false, // Mantém o tesouro atual
      );

      print('DEBUG: Sala ${i + 1} - Antes: monster1=${room.monster1}, monster2=${room.monster2}');
      print('DEBUG: Sala ${i + 1} - Depois: monster1=${updatedRoom.monster1}, monster2=${updatedRoom.monster2}');

      // Atualiza apenas os campos de monstros na sala existente
      dungeon.rooms[i] = room.copyWith(
        monster1: updatedRoom.monster1,
        monster2: updatedRoom.monster2,
      );
    }
  }
}
