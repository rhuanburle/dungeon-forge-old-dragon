...
class DungeonMapRenderer {
  ...
  Map<String, dynamic> buildGridWithPositions(Dungeon dungeon) {
    ...
    for (int i = 1; i < dungeon.roomsCount; i++) {
      bool placed = false;
      for (int attempt = 0; attempt < 150 && !placed; attempt++) {
        // Escolhe uma sala de referência
        final ref = centres[_rand.nextInt(centres.length)];
        
        // Escolhe direção mais inteligente
        final dir = _rand.nextInt(8); // 8 direções em vez de 4
  ...
  bool _canPlaceRoom(List<List<String>> grid, int x, int y, int w, int h) {
    ...
    // Verifica apenas se a área da sala está livre
    for (int i = y; i < y + h; i++) {
      for (int j = x; j < x + w; j++) {
  ...
  void _placeRoomOldSchool(
    ...
    // Escolhe formato: 0 = retângulo, 1 = L-shape, 2 = T-shape
    ...
    for (int i = 0; i < h; i++) {
      for (int j = 0; j < w; j++) {
        ...
        switch (shape) {
          case 1: // L-shape
            inside =
                (i < h * 0.7 && j < w * 0.7) || (i >= h * 0.3 && j >= w * 0.3);
            break;
          case 2: // T-shape
            inside = (i < h * 0.3) || (j >= w * 0.3 && j < w * 0.7);
            break;
