...
class DungeonMapRenderer {
  static const _gridSize = 200; // Aumentado para mais espaço
  static const _empty = ' ';
  static const _wall = '#';
  static const _floor = '.';
  static const _door = '+';
  ...
  Map<String, dynamic> buildGridWithPositions(Dungeon dungeon) {
    ...
    for (int i = 1; i < dungeon.roomsCount && candidateIndex < candidates.length; i++) {
      bool placed = false;
        ...
        for (int size = 0; size < 3; size++) {
          ...
          if (_canPlaceRoom(grid, x, y, w, h)) {
            ...
            for (final centre in centres) {
              final distance = sqrt(pow(centre.x - candidate.x, 2) + pow(centre.y - candidate.y, 2));
            ...
            placed = true;
            print('Sala ${i + 1} colocada no candidato $candidateIndex');
            break;
          }
        }
      }
      ...
      if (!placed) {
        print('FALHA: Não foi possível colocar a sala ${i + 1}');
      }
    }
