...
class RoomGenerationService {
  ...
  RoomGenerationDto generateRoom(
    ...
    final magicItem = _getMagicItem(col13Roll, col14Roll, col15Roll, type, monster, trap, specialTrap);
  ...
  /// Obtém item mágico baseado nas colunas 13, 14 e 15 com modificadores
  MagicItem? _getMagicItem(int col13Roll, int col14Roll, int col15Roll, RoomType type, Monster? monster, Trap? trap, SpecialTrap? specialTrap) {
    final roomTable = _tableManager.roomTable;
    final treasure = roomTable.getColumn13(col13Roll);

    // Aplica modificadores conforme a tabela 9.2
    // +1 para salas de armadilhas, +2 para salas de monstros
    int modifiedRoll = col15Roll;
    
    // Verifica se é sala de armadilha
    if (type == RoomType.trap || type == RoomType.specialTrap || trap != null || specialTrap != null) {
    ...
    if (treasure == Treasure.magicItem ||
      ...
      return roomTable.getColumn15(modifiedRoll);
    }
    return null;
  }
