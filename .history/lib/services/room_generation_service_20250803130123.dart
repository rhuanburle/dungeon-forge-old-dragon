...
class RoomGenerationService {
  ...
  RoomGenerationDto generateRoom(
    ...
    final treasure = _getTreasureWithModifiers(
      col13Roll,
      type,
      monster,
      trap,
  ...
  Monster? _getMonster(int col10Roll, String occupantI, String occupantII) {
    ...
    // Sempre obtém o monstro da coluna 10, independente do tipo de sala
  ...
  MagicItem? _getMagicItem(int col13Roll, int col14Roll, int col15Roll) {
    ...
    // Se o tesouro é magicItem diretamente
    if (treasure == Treasure.magicItem) {
      return roomTable.getColumn15(col15Roll);
    }

    // Se o tesouro é specialTreasure, verifica se resulta em item mágico
    if (treasure == Treasure.specialTreasure) {
      final specialTreasure = roomTable.getColumn14(col14Roll);

      // Apenas alguns specialTreasure resultam em itens mágicos
      if (specialTreasure == SpecialTreasure.rollAgainPlusMagicItem ||
          specialTreasure == SpecialTreasure.copperSilverGemsMagicItem ||
          specialTreasure == SpecialTreasure.rollAgainPlusMagicItem2) {
        return roomTable.getColumn15(col15Roll);
      }
    }
  ...
  Treasure _getTreasureWithModifiers(
    ...
    final roomTable = _tableManager.roomTable;
    final baseTreasure = roomTable.getColumn13(col13Roll);

    // TEMPORARIAMENTE: Retorna o tesouro base sem modificadores
    // para verificar a frequência base de itens mágicos
    return baseTreasure;
  }
