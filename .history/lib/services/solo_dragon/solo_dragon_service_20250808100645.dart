...
extension SoloDragonServiceRooms on SoloDragonService {
  ...
  String _getRoomType(RoomContext context, int roll) {
    final types = _roomTypes[context] ?? _roomTypes[RoomContext.enteringDungeon]!;
  ...
  String _resolveFormula(String formula) {
    ...
    // Exemplo: "1 em 1d6 para 1d6 PO" -> "1d6 PO"
    if (formula.contains('em 1d6 para')) {
      final parts = formula.split(' em 1d6 para ');
      final dice = parts[1];
      return dice;
...
const Map<int, RoomEvents> _roomEvents = {
  ...
  25: RoomEvents(
    ...
    trap: 'A porta de entrada ativa um portal para uma sala não explorada e aleatória',
    encounter: 'Bandidos',
    hasTreasure: true,
  ),
  26: RoomEvents(
  ...
  64: RoomEvents(
    ...
    content: 'Depósito de material de escavação. Pás, picaretas e muitas pedras',
