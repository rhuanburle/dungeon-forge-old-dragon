...
class SoloDragonService {
  ...
  /// Gera uma sala baseada nas tabelas A5.4 e A5.5
  SoloRoom generateRoom(RoomContext context) {
    // A5.4: Determina o tipo de cômodo
    final roomTypeRoll = rollD6() + rollD6(); // 2..12
    final roomType = _getRoomType(context, roomTypeRoll);

    // A5.5: Determina eventos (portas, conteúdo, armadilhas, encontros)
    final eventRoll = rollD6() + rollD6(); // 11..66
    final events = _getRoomEvents(eventRoll);

    // A5.6: Determina tesouros (se aplicável)
    String treasure = '';
    if (events.hasTreasure) {
      final treasureRoll = rollD6() + rollD6(); // 2..12
  ...
  RoomEvents _getRoomEvents(int roll) {
    final events = _roomEvents[roll];
    if (events == null) {
      return const RoomEvents(
...
const Set<int> _grayDoors = {
  11, 12, 14, 15, 21, 22, 24, 25, 31, 32, 34, 35, 41, 42, 44, 45, 51, 52, 54, 55, 61, 62, 64, 65,
};
...
extension SoloDragonServiceRooms on SoloDragonService {
  ...
  SoloRoom generateRoom(RoomContext context) {
    ...
    final doors = (_roomEvents[doorsRoll] ?? const RoomEvents(
      doors: '1 Porta de madeira',
      content: '',
      trap: '',
      encounter: '',
    ...
    if (_grayDoors.contains(doorsRoll)) {
      ...
      content = (_roomEvents[contentRoll] ?? const RoomEvents(
        doors: '',
        content: 'Vazio e sujo',
        trap: '',
        encounter: '',
      ...
      final isContentColored = _grayContent.contains(contentRoll) || _yellowContent.contains(contentRoll);

      if (_yellowContent.contains(contentRoll)) {
        treasureTriggeredFromContent = true;
        treasureRoll = rollD6() + rollD6();
      }

      if (isContentColored) {
        trapTriggeredFromContent = true;
        trapRoll = rollD6() * 10 + rollD6();
        trap = (_roomEvents[trapRoll] ?? const RoomEvents(
          doors: '',
          content: '',
          trap: '',
          encounter: '',
        ...
        if (_grayTraps.contains(trapRoll)) {
          ...
          encounter = (_roomEvents[encounterRoll] ?? const RoomEvents(
            doors: '',
            content: '',
            trap: '',
            encounter: '',
