...
class SoloDragonService {
  ...
  /// Rola d6+d6 como dezena e unidade (11..66)
  int rollD6D6() => rollD6() * 10 + rollD6();

  /// Retorna todos os resultados possíveis de d6+d6 (11..66) em ordem crescente
  List<int> allEventRolls() {
    final List<int> rolls = [];
    for (int tens = 1; tens <= 6; tens++) {
      for (int units = 1; units <= 6; units++) {
        rolls.add(tens * 10 + units);
      }
    }
    return rolls;
  }

  /// Retorna o texto da tabela A5.5 para o roll informado e a coluna desejada
  String getEventTextForRoll(int roll, EventColumn column) {
    final events = _roomEvents[roll];
    if (events == null) return '';
    switch (column) {
      case EventColumn.doors:
        return events.doors;
      case EventColumn.content:
        return events.content;
      case EventColumn.trap:
        return events.trap;
      case EventColumn.encounter:
  ...
  FinalChamberResult getFinalChamberForRolls(int aRoll, int subRoll) {
    ...
    final subMap = switch (a.subTable) { 'B' => _finalB, 'C' => _finalC, 'D' => _finalD, _ => _finalC };
  ...
  FinalChamberResult rollFinalChamber() {
    final aRoll = rollD6();
    final subTable = _finalA[aRoll]?.subTable ?? 'C';
...
/// Colunas possíveis da tabela de eventos A5.5
enum EventColumn { doors, content, trap, encounter }
...
class RumorBoard {
  ...
  void _eliminateWithMigration(ColumnId column, int rowIndex) {
    // Primeiro, tenta eliminar a célula especificada
    if (!_isEliminated(column, rowIndex) && _remainingCount(column) > 1) {
      _setEliminated(column, rowIndex);
      return;
    }

    // Se não pode eliminar a célula especificada, aplica migração
    final order = <_Cell>[
      _Cell(ColumnId.a, 0),
      _Cell(ColumnId.a, 1),
      _Cell(ColumnId.a, 2),
    ...
    for (int i = 1; i < order.length; i++) {
      ...
      // Não eliminar se é a única célula restante em sua coluna (descoberta)
      if (_remainingCount(current.column) == 1) {
        continue;
      }
...
String getTrapDetail(String trapText) {
  ...
  if (lower.contains('alarme') || lower.contains('sinet')) {
    return 'Alarme dispara sinetas escondidas e amarradas com cordões por toda a sala, alertando a todos num raio de 50 metros e impedindo que sejam surpreendidos.';
  }
  if (lower.contains('bloco') && lower.contains('cai')) {
    return 'Bloco que Cai do teto da masmorra. Causa 1d10 de dano aos atingidos.';
  }
  if (lower.contains('dardo')) {
    return 'Dardos Envenenados: 1d6 dardos são expelidos por buracos nas paredes e causam 1d4 de dano cada. Defina o efeito do veneno.';
  }
  if (lower.contains('desmoronament') || lower.contains('pedra')) {
    return 'Desmoronamento de pedras interrompe o caminho. Remoção leva 3d6+10 turnos, ou retorne e tente outra rota.';
  }
  if (lower.contains('fosso')) {
    return 'Fosso com estacas causa 2d6 de dano a quem cair. 1-2 em 1d6 de não haver estacas e sim uma passagem a um nível inferior.';
  }
  if (lower.contains('guilhotina')) {
    return 'Guilhotina Oculta corta com lâmina rápida. Causa 1d8 de dano.';
  }
  if (lower.contains('poço') && lower.contains('água')) {
    return 'Poço de Água suspeita no centro da sala. Pode ser água potável, profana, benta ou envenenada.';
  }
  if (lower.contains('porta secreta')) {
    return 'Porta Secreta escondida. Só revele se for descoberta por um personagem; caso contrário, trate como sala vazia.';
  }
  if (lower.contains('portal')) {
    return 'Portal Dimensional: pode enviar os personagens para salas aleatórias da masmorra, atrapalhando mapas.';
  }
  if (lower.contains('spray') || lower.contains('ácido')) {
    return 'Spray Ácido atinge todos até 6 metros. Causa 1d4 de dano ácido.';
  }
  if (lower.contains('teto') && lower.contains('retr')) {
    return 'Teto Retrátil desce até esmagar todos. Exige Desarmar Armadilhas por um Ladrão em até 1d8 turnos.';
  }
  // fallback genérico
  return trapText;
}
