...
class SoloDragonService {
  ...
  // ==========================
  // Opcional: Encontros 1d20, Cidade e Viagem
  // ==========================
  OptionalEncounterResult rollOptionalEncounter1d20() {
    final r = _random.nextInt(20) + 1; // 1..20
    bool leader = false;
    int base = r;
    if (r == 19 || r == 20) {
      leader = true;
      base = _random.nextInt(18) + 1; // 1..18
    }
    final name = _optionalEncounter1d20[base] ?? 'Criatura';
    return OptionalEncounterResult(roll: r, name: name, isLeader: leader);
  }

  String rollTravelEventD6() => _travelEvents[rollD6()] ?? '';
  String rollCityEventD6() => _cityEvents[rollD6()] ?? '';

  bool shouldCheckRandomEncounter(int turnCount) => turnCount % 2 == 0;
  bool rollRandomEncounterChance({required bool noisyLight}) {
    final r = rollD6();
    return noisyLight ? r <= 2 : r == 1;
  }
...
// ==========================
// Opcional: Tabela de Encontros 1d20
// ==========================
class OptionalEncounterResult {
  final int roll; // 1..20
  final String name;
...
const Map<int, String> _optionalEncounter1d20 = {
  ...
  17: 'URSO-CORUJA',
  18: 'ZUMBIS',
  // 19/20: Role novamente + Líder (tratados na lógica)
};

// ==========================
// Opcional: Eventos de Viagem (Tabela 2) e Cidade (Tabela 3)
// ==========================
const Map<int, String> _travelEvents = {
  1: 'Encontro (role na Tabela 6).',
  2: 'Obstáculo no Caminho: +1 dia de viagem; consuma 1 ração.',
  3: 'Tempestade ou Clima Hostil: -1 PV.',
  4: 'Pegadas frescas: teste de Sabedoria para evitar surpresa na próxima batalha.',
  5: 'Viagem tranquila.',
  6: 'Encontram abrigo seguro: Cura 1d4 PV.',
};

const Map<int, String> _cityEvents = {
  1: 'Nova caravana: 10% de desconto na compra de itens.',
  2: 'Aumento de impostos: preços sobem 25% nesta estadia.',
  3: 'Festa/feira: chance de NPC amigável que oferece ajuda.',
  4: 'Bando de ladrões: 1d6 em 1-2 perde 1d6x10 PO; 3-6 nada acontece.',
  5: 'Recompensa por criatura específica: se surgir na próxima masmorra, vale o dobro do tesouro.',
  6: 'Pequena epidemia: cura custa o dobro.',
};
