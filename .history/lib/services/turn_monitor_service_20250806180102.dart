...
class TurnMonitorService {
  ...
  TurnResult testLockedDoor({bool hasThiefTools = false, bool isThief = false}) {
    final roll = DiceRoller.rollStatic(1, 6);
    ...
    if (success) {
      ...
      return const TurnResult(
        success: false,
        description: 'Falha ao abrir a porta. Tentativa eliminou possibilidade de surpresa.',
        doorOpened: false,
      );
    }
  }
  ...
  bool checkExhaustion() {
    ...
    return _explorationTurns > 0 && (_explorationTurns % 5) == 0 && _restTurns < (_explorationTurns / 5).floor();
  }
  ...
  String exportToText() {
    ...
    buffer.writeln('Tocha: ${_groupData.hasTorch ? "Sim (${_groupData.torchTurns} turnos restantes)" : "Não"}');
    buffer.writeln('Lanterna: ${_groupData.hasLantern ? "Sim (${_groupData.lanternTurns} turnos restantes)" : "Não"}');
