...
class TreasureResolver {
  static String resolve(String template) {
    ...
    for (var rawPart in parts) {
      ...
      if (match != null) {
        ...
        if (unit == 'Gemas') {
          ...
          // Sempre inclui a unidade, mesmo que seja apenas um número
          if (unit.isNotEmpty) {
            results.add('$value ${unit.trim()}'.trim());
          } else {
            // Se não há unidade, ignora o resultado
            print('Ignoring result without unit: $value from "$part"');
            continue;
          }
        }
      } else {
        ...
        if (numberMatch != null) {
          ...
          if (unit.isNotEmpty) {
            ...
            print('Ignoring standalone number: "$part"');
            continue;
          }
        } else {
    ...
    final result = cleanResults.join(' + ');
