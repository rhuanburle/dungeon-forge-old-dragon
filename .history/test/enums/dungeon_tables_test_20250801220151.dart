  ...
  group('Dungeon Enums', () {
    group('DungeonType', () {
      test('should have correct descriptions', () {
        expect(DungeonType.lostConstruction.description,
            equals('Construção Perdida'));
        expect(DungeonType.artificialLabyrinth.description,
            equals('Labirinto Artificial'));
        expect(
            DungeonType.naturalCaves.description, equals('Cavernas Naturais'));
        expect(
            DungeonType.abandonedLair.description, equals('Covil Desabitado'));
        expect(DungeonType.abandonedFortress.description,
            equals('Fortaleza Abandonada'));
        expect(
            DungeonType.deactivatedMine.description, equals('Mina Desativada'));
      });
    ...
    group('DungeonBuilder', () {
      test('should have correct descriptions', () {
        ...
        expect(DungeonBuilder.ancestralCivilization.description,
            equals('Civilização Ancestral'));
    ...
    group('DungeonStatus', () {
      test('should have correct descriptions', () {
        ...
        expect(DungeonStatus.inAnotherLocation.description,
            equals('em outro local'));
      });
    ...
    group('DungeonTargetStatus', () {
      test('should have correct descriptions', () {
        expect(DungeonTargetStatus.beingSought.description,
            equals('sendo procurado'));
        expect(DungeonTargetStatus.destroyed.description, equals('destruído'));
        expect(DungeonTargetStatus.disappeared.description,
            equals('desaparecido'));
    ...
    group('DungeonLocation', () {
      test('should have correct descriptions', () {
        expect(DungeonLocation.scorchingDesert.description,
            equals('Deserto Escaldante'));
        expect(DungeonLocation.underCity.description, equals('Sob uma Cidade'));
        expect(DungeonLocation.frozenMountain.description,
            equals('Montanha Gelada'));
        expect(DungeonLocation.wildForest.description,
            equals('Floresta Selvagem'));
        expect(
            DungeonLocation.fetidSwamp.description, equals('Pântano Fétido'));
        expect(
            DungeonLocation.isolatedIsland.description, equals('Ilha Isolada'));
      });
    ...
    group('DungeonEntry', () {
      test('should have correct descriptions', () {
        expect(DungeonEntry.behindWaterfall.description,
            equals('Atrás de uma Cachoeira'));
        ...
        expect(
            DungeonEntry.rockFissure.description, equals('Fissura numa Rocha'));
        expect(DungeonEntry.monsterLair.description,
            equals('Covil de um Monstro'));
        expect(
            DungeonEntry.volcanoMouth.description, equals('Boca de um Vulcão'));
      });
    ...
    group('DungeonOccupant', () {
      test('should have correct descriptions', () {
        ...
        expect(
            DungeonOccupant.lizardMen.description, equals('Homens Lagartos'));
        expect(DungeonOccupant.hobgoblin.description, equals('Hobgoblin'));
        expect(DungeonOccupant.gelatinousCube.description,
            equals('Cubo Gelatinoso'));
    ...
    group('RumorSubject', () {
      test('should have correct descriptions', () {
        expect(RumorSubject.decapitatedOccupant.description,
            equals('Um/uma [coluna 11] decapitada/o'));
        expect(RumorSubject.drunkPeasant.description,
            equals('Um camponês bêbado'));
        expect(RumorSubject.primaryOccupant.description,
            equals('Um/uma [coluna 10]'));
        expect(RumorSubject.richForeigner.description,
            equals('Um estrangeiro muito rico'));
    ...
    group('RumorAction', () {
      test('should have correct descriptions', () {
        ...
        expect(
            RumorAction.capturedIn.description, equals('foi capturado na/no'));
        expect(RumorAction.leftTrailsIn.description,
            equals('deixou rastros na/no'));
        expect(RumorAction.soughtPriestIn.description,
            equals('procurou o sacerdote na/no'));
        expect(RumorAction.killedByWerewolfIn.description,
            equals('foi morto por um lobisomem na/no'));
    ...
    group('RumorLocation', () {
      test('should have correct descriptions', () {
        expect(RumorLocation.autumnReligiousFestival.description,
            equals('festival religioso do outono'));
        expect(RumorLocation.villageLastYearDuringEclipse.description,
            equals('vila no ano passado durante o eclipse'));
        expect(RumorLocation.farmWhenSheepDisappeared.description,
            equals('fazenda quando uma ovelha sumiu'));
        expect(RumorLocation.nearbyVillage.description,
            equals('aldeia vizinha próxima'));
        expect(RumorLocation.springTradeCaravan.description,
            equals('caravana de comércio da primavera'));
        expect(RumorLocation.winterBlizzard3YearsAgo.description,
            equals('nevasca do inverno há 3 anos'));
      });
