// test/integration/dungeon_generator_integration_test.dart
  ...
  group('Dungeon Generator Integration Tests', () {
    ...
    group('complete generation flow', () {
      ...
      test('should generate different dungeons on multiple calls', () {
        ...
        for (int i = 0; i < 5; i++) {
          dungeons.add(generator.generate(level: 3, theme: 'Diversity Test $i'));
        }

        // Should have some variation in types
        final types = dungeons.map((d) => d.type).toSet();
        expect(types.length, greaterThan(1), reason: 'Should generate different dungeon types');
        
        // Should have some variation in locations
        final locations = dungeons.map((d) => d.location).toSet();
        expect(locations.length, greaterThan(1), reason: 'Should generate different locations');
      });
    ...
    group('room generation validation', () {
      ...
      test('should generate rooms with different types', () {
        ...
        expect(roomTypes.length, greaterThan(1), reason: 'Should generate different room types');
      });
      ...
      test('should generate proper monster encounters', () {
        ...
        final monsterRooms = dungeon.rooms.where((r) => r.type.contains('Encontro')).toList();
        
        for (final room in monsterRooms) {
          // Monster rooms should have at least one monster
          expect(room.monster1.isNotEmpty || room.monster2.isNotEmpty, isTrue);
      ...
      test('should generate proper trap rooms', () {
        ...
        final trapRooms = dungeon.rooms.where((r) => 
          r.type.contains('Armadilha') || r.type.contains('Trap')).toList();
        
        for (final room in trapRooms) {
          // Trap rooms should have trap descriptions or be properly identified
          final hasTrapDescription = room.trap.isNotEmpty || 
                                   room.specialTrap.isNotEmpty ||
                                   room.type.contains('Armadilha') ||
                                   room.type.contains('Especial');
          expect(hasTrapDescription, isTrue, 
                reason: 'Trap room should have trap description or be identified as trap: ${room.type}');
    ...
    group('business rules validation', () {
      test('should apply treasure modifiers for trap rooms', () {
        ...
        if (totalTrapRooms > 0) {
          ...
          expect(treasureRate, greaterThan(0.1), 
                reason: 'Trap rooms should have reasonable treasure rate due to +1 modifier');
        }
      });
      ...
      test('should apply treasure modifiers for monster rooms', () {
        ...
        if (totalMonsterRooms > 0) {
          ...
          expect(treasureRate, greaterThan(0.2), 
                reason: 'Monster rooms should have reasonable treasure rate due to +2 modifier');
        }
      });
    ...
    group('edge cases and stress tests', () {
      ...
      test('should handle empty theme', () {
        final dungeon = generator.generate(level: 5, theme: '');
        _validateDungeonProperties(dungeon);
      });
    });
  });
...
void _validateDungeonProperties(Dungeon dungeon) {
  ...
  expect(dungeon.builderOrInhabitant, isNotEmpty, reason: 'Builder/inhabitant should not be empty');
  expect(dungeon.status, isNotEmpty, reason: 'Status should not be empty');
  expect(dungeon.objective, isNotEmpty, reason: 'Objective should not be empty');
  ...
  expect(dungeon.occupant1, isNotEmpty, reason: 'Occupant1 should not be empty');
  expect(dungeon.occupant2, isNotEmpty, reason: 'Occupant2 should not be empty');
  ...
  expect(dungeon.roomsCount, greaterThan(0), reason: 'Should have at least one room');
  expect(dungeon.rooms.length, equals(dungeon.roomsCount), reason: 'Room count should match rooms list length');
}
...
void _validateRoomProperties(Room room) {
  ...
  expect(room.type, isNot(contains('…')), reason: 'Room type should not contain unresolved references');
