...
void main() {
  group('DungeonGenerationDto', () {
    ...
    test('should create instance with all required properties', () {
      ...
      expect(dto.builder, equals(DungeonBuilder.unknown));
      ...
      expect(dto.size, equals(DungeonSize.large));
      expect(dto.sizeFormula, equals('Grande – 3d6+6'));
      expect(dto.occupantI, equals(DungeonOccupant.trolls));
      expect(dto.occupantII, equals(DungeonOccupant.kobolds));
      expect(dto.leader, equals(DungeonOccupant.hobgoblin));
    ...
    test('fullObjective should combine objective, target and targetStatus', () {
      final expected = 'Defender artefato sendo procurado';
      expect(dto.fullObjective, equals(expected));
    });

    test('fullRumor should combine rumor components', () {
      final expected = 'Um camponês bêbado foi visto próximo a festival religioso do outono';
    ...
    test('resolveRumorReferences should replace column references', () {
      ...
      final resolved = dtoWithReferences.resolveRumorReferences(
        'Trolls',
        'Kobolds', 
        'Hobgoblin'
      );
  ...
  group('RoomGenerationDto', () {
    ...
    test('should create instance with all properties', () {
      ...
      expect(dto.specialTreasure, equals(SpecialTreasure.rollAgainPlusMagicItem));
      expect(dto.magicItem, equals(MagicItem.any1));
    });

    test('hasTrap should return true for trap and specialTrap types', () {
      final trapDto = dto.copyWith(type: RoomType.trap);
      final specialTrapDto = dto.copyWith(type: RoomType.specialTrap);
      final monsterDto = dto.copyWith(type: RoomType.monster);

      expect(trapDto.hasTrap, isTrue);
      expect(specialTrapDto.hasTrap, isTrue);
      expect(monsterDto.hasTrap, isFalse);
    });
    ...
    test('hasMonster should return true for monster type', () {
      ...
      expect(monsterDto.hasMonster, isTrue);
      expect(trapDto.hasMonster, isFalse);
    });

    test('treasureModifier should return correct values', () {
      final trapDto = dto.copyWith(type: RoomType.trap);
      final specialTrapDto = dto.copyWith(type: RoomType.specialTrap);
      final monsterDto = dto.copyWith(type: RoomType.monster);
      final commonDto = dto.copyWith(type: RoomType.commonRoom);

      expect(trapDto.treasureModifier, equals(1));
      expect(specialTrapDto.treasureModifier, equals(1));
      expect(monsterDto.treasureModifier, equals(2));
      expect(commonDto.treasureModifier, equals(0));
    });
  });
  ...
  group('TreasureDto', () {
    ...
    test('hasValue should return true when any value is present', () {
      ...
      const dtoWithValuable = TreasureDto(description: 'Test', valuableObjects: 2);
      const dtoWithMagic = TreasureDto(description: 'Test', magicItem: 'Sword +1');
