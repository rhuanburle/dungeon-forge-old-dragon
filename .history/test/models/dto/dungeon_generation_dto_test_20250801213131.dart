...
void main() {
  group('DungeonGenerationDto', () {
    test('should create a valid DTO with all required fields', () {
    ...
    test('should generate correct fullObjective', () {
      final dto = DungeonGenerationDto(
        type: DungeonType.lostConstruction,
        builderOrInhabitant: DungeonBuilder.unknown,
        status: DungeonStatus.lost,
      ...
      expect(dto.fullObjective, equals('Defender tesouro que está intacto'));
    });

    test('should generate correct fullRumor with resolved references', () {
      final dto = DungeonGenerationDto(
        type: DungeonType.lostConstruction,
        builderOrInhabitant: DungeonBuilder.unknown,
        status: DungeonStatus.lost,
        ...
        occupantII: 'Kobolds',
        leader: 'Hobgoblin',
        rumorSubject: RumorSubject.decapitatedOccupant,
        rumorAction: RumorAction.seenNear,
        rumorLocation: RumorLocation.nearbyVillage,
      );

      expect(dto.fullRumor, equals('Um/uma Kobolds decapitada/o foi visto próximo a aldeia vizinha próxima'));
    });

    test('should create a copy with new values', () {
      final original = DungeonGenerationDto(
        type: DungeonType.lostConstruction,
        builderOrInhabitant: DungeonBuilder.unknown,
        status: DungeonStatus.lost,
        objective: DungeonObjective.defend,
      ...
      final copy = original.copyWith(
        type: DungeonType.artificialLabyrinth,
        location: DungeonLocation.scorchingDesert,
      );

      expect(copy.type, equals(DungeonType.artificialLabyrinth));
      expect(copy.location, equals(DungeonLocation.scorchingDesert));
      expect(copy.builderOrInhabitant, equals(original.builderOrInhabitant));
