...
void main() {
  group('Dungeon Generator Regression Tests', () {
    group('functionality comparison', () {
      test('should generate dungeons with same basic structure', () {
        ...
        for (int i = 0; i < 10; i++) {
          ...
          expect(original.type, isNotEmpty);
          expect(refactored.type, isNotEmpty);
          
          expect(original.builderOrInhabitant, isNotEmpty);
          expect(refactored.builderOrInhabitant, isNotEmpty);
          
          expect(original.status, isNotEmpty);
          expect(refactored.status, isNotEmpty);
          
          expect(original.objective, isNotEmpty);
          expect(refactored.objective, isNotEmpty);
          
          expect(original.location, isNotEmpty);
          expect(refactored.location, isNotEmpty);
          
          expect(original.entry, isNotEmpty);
          expect(refactored.entry, isNotEmpty);
          
          expect(original.occupant1, isNotEmpty);
          expect(refactored.occupant1, isNotEmpty);
          
          expect(original.occupant2, isNotEmpty);
          expect(refactored.occupant2, isNotEmpty);
          
          expect(original.leader, isNotEmpty);
          expect(refactored.leader, isNotEmpty);
          
          expect(original.rumor1, isNotEmpty);
          expect(refactored.rumor1, isNotEmpty);

          // Room count should be reasonable
          expect(original.roomsCount, greaterThanOrEqualTo(3));
          expect(refactored.roomsCount, greaterThanOrEqualTo(3));
          
          expect(original.rooms.length, equals(original.roomsCount));
          expect(refactored.rooms.length, equals(refactored.roomsCount));
        }
      });
    ...
    group('room generation validation', () {
      test('should generate rooms with all required properties', () {
        ...
        final original = originalGenerator.generate(level: 5, theme: 'Room Test');
        final refactored = refactoredGenerator.generate(level: 5, theme: 'Room Test');
      ...
      test('should generate rooms with valid types', () {
        ...
        for (int i = 0; i < 20; i++) {
          final dungeon = refactoredGenerator.generate(level: 3, theme: 'Type Test');
          ...
          for (final room in dungeon.rooms) {
            ...
            final hasValidType = validTypes.any((type) => room.type.contains(type));
            expect(hasValidType, isTrue, reason: 'Invalid room type: ${room.type}');
          }
        }
      });
    });
    ...
    group('business rule validation', () {
      test('should apply treasure modifiers correctly', () {
        ...
        for (int i = 0; i < 50; i++) {
          final dungeon = refactoredGenerator.generate(level: 5, theme: 'Modifier Test');
          
          for (final room in dungeon.rooms) {
            totalRooms++;
            
            if (room.type.contains('Armadilha') && room.treasure.isNotEmpty && room.treasure != 'Nenhum') {
              trapRoomsWithTreasure++;
            }
            
            if (room.type.contains('Encontro') && room.treasure.isNotEmpty && room.treasure != 'Nenhum') {
      ...
      test('should resolve special references correctly', () {
        ...
        for (int i = 0; i < 30; i++) {
          final dungeon = refactoredGenerator.generate(level: 4, theme: 'Reference Test');
      ...
      test('should generate valid occupant substitutions', () {
        ...
        for (int i = 0; i < 20; i++) {
          final dungeon = refactoredGenerator.generate(level: 6, theme: 'Occupant Test');
    ...
    group('edge cases and error handling', () {
      test('should handle minimum level correctly', () {
        ...
        expect(() => originalGenerator.generate(level: 1, theme: 'Min Level'), returnsNormally);
        expect(() => refactoredGenerator.generate(level: 1, theme: 'Min Level'), returnsNormally);
      });
      ...
      test('should handle high level correctly', () {
        ...
        expect(() => originalGenerator.generate(level: 20, theme: 'High Level'), returnsNormally);
        expect(() => refactoredGenerator.generate(level: 20, theme: 'High Level'), returnsNormally);
      });
      ...
      test('should handle empty theme', () {
        ...
        expect(() => refactoredGenerator.generate(level: 3, theme: ''), returnsNormally);
      });
    });
    ...
    group('integration with TreasureResolver', () {
      test('should resolve treasure formulas correctly', () {
        ...
        for (int i = 0; i < 30; i++) {
          final dungeon = refactoredGenerator.generate(level: 5, theme: 'Treasure Test');
          
          for (final room in dungeon.rooms) {
            // Treasures should not contain unresolved dice formulas
            if (room.treasure.isNotEmpty && room.treasure != 'Nenhum' && room.treasure != 'Nenhum Tesouro') {
            ...
            if (room.specialTreasure.isNotEmpty) {
              expect(room.specialTreasure, isNot(matches(r'\d+d\d+')),
                     reason: 'Unresolved dice formula in special treasure: ${room.specialTreasure}');
            }
