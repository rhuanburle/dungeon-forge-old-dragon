...
void main() {
  ...
  group('Magic Item frequency validation', () {
    test('should have correct magic item frequency according to table 9.2', () {
      ...
      print('Magic item frequency: ${(magicItemFrequency * 100).toStringAsFixed(1)}%');
    ...
    test('should apply treasure modifiers correctly for trap and monster rooms', () {
      ...
      for (int i = 0; i < 20; i++) {
        ...
        for (final room in dungeon.rooms) {
          final hasMagicItem = room.magicItem.isNotEmpty && room.magicItem != '—';
      ...
      // Calcula frequências
      final trapMagicFrequency = totalTrapRooms > 0 ? trapRoomsWithMagicItems / totalTrapRooms : 0;
      final monsterMagicFrequency = totalMonsterRooms > 0 ? monsterRoomsWithMagicItems / totalMonsterRooms : 0;
      final normalMagicFrequency = totalNormalRooms > 0 ? normalRoomsWithMagicItems / totalNormalRooms : 0;
      
      print('Treasure Modifier Statistics:');
      print('Trap rooms with magic items: $trapRoomsWithMagicItems / $totalTrapRooms (${(trapMagicFrequency * 100).toStringAsFixed(1)}%)');
      print('Monster rooms with magic items: $monsterRoomsWithMagicItems / $totalMonsterRooms (${(monsterMagicFrequency * 100).toStringAsFixed(1)}%)');
      print('Normal rooms with magic items: $normalRoomsWithMagicItems / $totalNormalRooms (${(normalMagicFrequency * 100).toStringAsFixed(1)}%)');
