...
void main() {
  group('Dungeon Generator Tests', () {
    ...
    group('edge cases and error handling', () {
      ...
      test('should handle empty theme', () {
        ...
        expect(() => generator.generate(level: 3, theme: ''), returnsNormally);
      });
    });
  ...
  group('Magic Item frequency validation', () {
    test('should have correct magic item frequency according to table 9.2', () {
      final generator = DungeonGeneratorRefactored();

      // Gera múltiplas masmorras para estatísticas
      int totalRooms = 0;
      int roomsWithMagicItems = 0;
      int trapRooms = 0;
      int monsterRooms = 0;

      for (int i = 0; i < 10; i++) {
        final dungeon = generator.generate(level: 3, theme: 'Magic Item Test');

        for (final room in dungeon.rooms) {
          totalRooms++;

          // Verifica se tem item mágico
          if (room.magicItem.isNotEmpty && room.magicItem != '—') {
            roomsWithMagicItems++;
          }

          // Conta salas de armadilha e monstro
      ...
      // Calcula frequência
      final magicItemFrequency = roomsWithMagicItems / totalRooms;

      // Conforme a tabela 9.2, itens mágicos aparecem em:
      // - Coluna 13: Treasure.magicItem (rolls 9-12)
      // - Coluna 14: SpecialTreasure.rollAgainPlusMagicItem (rolls 2-3, 11-12)
      // - Com modificadores: +1 para armadilhas, +2 para monstros

      // Frequência esperada: aproximadamente 25-35% das salas
      expect(magicItemFrequency, greaterThan(0.15)); // Mínimo 15%
      expect(magicItemFrequency, lessThan(0.45)); // Máximo 45%

      print('Magic Item Statistics:');
      print('Total rooms: $totalRooms');
      print('Rooms with magic items: $roomsWithMagicItems');
      print(
        'Magic item frequency: ${(magicItemFrequency * 100).toStringAsFixed(1)}%',
      );
      print('Trap rooms: $trapRooms');
      print('Monster rooms: $monsterRooms');
    });

    test('should apply treasure modifiers correctly for trap and monster rooms', () {
      final generator = DungeonGeneratorRefactored();

      // Gera masmorras e verifica se salas de armadilha/monstro têm mais itens mágicos
      int trapRoomsWithMagicItems = 0;
      int monsterRoomsWithMagicItems = 0;
      int normalRoomsWithMagicItems = 0;
      int totalTrapRooms = 0;
      int totalMonsterRooms = 0;
      int totalNormalRooms = 0;

      for (int i = 0; i < 20; i++) {
        final dungeon = generator.generate(level: 3, theme: 'Modifier Test');

        for (final room in dungeon.rooms) {
          final hasMagicItem =
              room.magicItem.isNotEmpty && room.magicItem != '—';
          final hasTrap = room.trap.isNotEmpty && room.trap != '—';
          final hasMonster = room.monster1.isNotEmpty && room.monster1 != '—';

          if (hasTrap) {
            totalTrapRooms++;
            if (hasMagicItem) trapRoomsWithMagicItems++;
          } else if (hasMonster) {
            totalMonsterRooms++;
            if (hasMagicItem) monsterRoomsWithMagicItems++;
          } else {
            totalNormalRooms++;
            if (hasMagicItem) normalRoomsWithMagicItems++;
          }
        }
      }

      // Calcula frequências
      final trapMagicFrequency = totalTrapRooms > 0
          ? trapRoomsWithMagicItems / totalTrapRooms
          : 0;
      final monsterMagicFrequency = totalMonsterRooms > 0
          ? monsterRoomsWithMagicItems / totalMonsterRooms
          : 0;
      final normalMagicFrequency = totalNormalRooms > 0
          ? normalRoomsWithMagicItems / totalNormalRooms
          : 0;

      print('Treasure Modifier Statistics:');
      print(
        'Trap rooms with magic items: $trapRoomsWithMagicItems / $totalTrapRooms (${(trapMagicFrequency * 100).toStringAsFixed(1)}%)',
      );
      print(
        'Monster rooms with magic items: $monsterRoomsWithMagicItems / $totalMonsterRooms (${(monsterMagicFrequency * 100).toStringAsFixed(1)}%)',
      );
      print(
        'Normal rooms with magic items: $normalRoomsWithMagicItems / $totalNormalRooms (${(normalMagicFrequency * 100).toStringAsFixed(1)}%)',
      );

      // Salas de armadilha e monstro devem ter frequência maior de itens mágicos
      // devido aos modificadores +1 e +2
      if (totalTrapRooms > 5 && totalMonsterRooms > 5 && totalNormalRooms > 5) {
        expect(trapMagicFrequency, greaterThan(normalMagicFrequency * 0.8));
        expect(monsterMagicFrequency, greaterThan(normalMagicFrequency * 0.8));
      }
    });
  });
}
