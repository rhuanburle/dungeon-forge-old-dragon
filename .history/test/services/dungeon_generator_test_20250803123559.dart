...
void main() {
  group('DungeonGenerator Tests', () {
    ...
    test('should sync encounter difficulty with treasure level automatically', () {
      // Testa sincronização Iniciante -> Nível 1
      final dungeonIniciante = generator.generate(
        level: 3,
        theme: 'Test Theme',
        customRoomCount: 3,
        partyLevel: PartyLevel.iniciante,
      ...
      final dungeonAvancado = generator.generate(
        ...
        theme: 'Test Theme',
        customRoomCount: 3,
        partyLevel: PartyLevel.avancado,
        useEncounterTables: true,
        useTreasureByLevel: true, // Deve ser ativado automaticamente
      );
      ...
      for (final room in dungeonAvancado.rooms) {
        if (room.treasure.isNotEmpty && room.treasure.contains('Itens Mágicos:')) {
          hasMagicItems = true;
          break;
        }
      }
      // Pelo menos uma sala deve ter tesouro
      expect(dungeonAvancado.rooms.any((room) => room.treasure.isNotEmpty), isTrue);
    });
    ...
    test('should allow manual override of auto-synced treasure settings', () {
      ...
      final dungeonManual = generator.generate(
        ...
        theme: 'Test Theme',
        customRoomCount: 3,
        partyLevel: PartyLevel.iniciante, // Deveria sincronizar com nível 1
        useEncounterTables: true,
      ...
      // Pelo menos uma sala deve ter tesouro
      expect(dungeonManual.rooms.any((room) => room.treasure.isNotEmpty), isTrue);
    });
  });
}
