  ...
  group('ExplorationService', () {
    ...
    group('Teste de Descobertas Ancestrais - Tabela 4.4', () {
      ...
      test('deve gerar ruínas quando tipo ancestral for ruins', () {
        final result = service.generateRuin();

        expect(result, isNotNull);
        expect(result.type, isA<RuinType>());
        expect(result.description, isNotEmpty);
        expect(result.details, isNotEmpty);
      });

      test('deve gerar relíquias quando tipo ancestral for relics', () {
        final result = service.generateRelic();

        expect(result, isNotNull);
        expect(result.type, isA<RelicType>());
        expect(result.description, isNotEmpty);
        expect(result.details, isNotEmpty);
      });

      test('deve gerar objetos quando tipo ancestral for objects', () {
        final result = service.generateObject();

        expect(result, isNotNull);
        expect(result.type, isA<ObjectType>());
        expect(result.description, isNotEmpty);
        expect(result.details, isNotEmpty);
      });

      test('deve gerar vestígios quando tipo ancestral for vestiges', () {
        final result = service.generateVestige();

        expect(result, isNotNull);
        expect(result.type, isA<VestigeType>());
        expect(result.description, isNotEmpty);
        expect(result.details, isNotEmpty);
      });

      test('deve gerar ossadas quando tipo ancestral for ossuaries', () {
        final result = service.generateOssuary();

        expect(result, isNotNull);
        expect(result.type, isA<OssuaryType>());
        expect(result.description, isNotEmpty);
        expect(result.details, isNotEmpty);
      });
      ...
      test(
        ...
        () {
          final result = service.generateMagicalItem();

          expect(result, isNotNull);
          expect(result.type, isA<MagicalItemType>());
          expect(result.description, isNotEmpty);
          expect(result.details, isNotEmpty);
        },
      );
    ...
    group('Teste de Detalhamento Completo dos Ermos', () {
      test('deve gerar detalhes completos para descobertas ancestrais', () {
      ...
      test('deve gerar detalhes completos para ruínas', () {
        final result = service.generateDetailedRuin();

        expect(result, isNotNull);
        expect(result.type, isA<RuinType>());
        expect(result.description, isNotEmpty);
        expect(result.details, isNotEmpty);
        expect(result.size, isNotEmpty);
        expect(result.defenses, isNotEmpty);
      });

      test('deve gerar detalhes completos para relíquias', () {
        final result = service.generateDetailedRelic();

        expect(result, isNotNull);
        expect(result.type, isA<RelicType>());
        expect(result.description, isNotEmpty);
        expect(result.details, isNotEmpty);
        expect(result.condition, isNotEmpty);
      });

      test('deve gerar detalhes completos para objetos', () {
        final result = service.generateDetailedObject();
      ...
      test('deve gerar detalhes completos para vestígios', () {
        final result = service.generateDetailedVestige();
      ...
      test('deve gerar detalhes completos para ossadas', () {
        final result = service.generateDetailedOssuary();
      ...
      test('deve gerar detalhes completos para itens mágicos', () {
        final result = service.generateDetailedMagicalItem();

        expect(result, isNotNull);
        expect(result.type, isA<MagicalItemType>());
        expect(result.description, isNotEmpty);
        expect(result.details, isNotEmpty);
        expect(result.power, isNotEmpty);
      });

      test('deve gerar detalhes completos para covis', () {
        final result = service.generateDetailedLair();
      ...
      test('deve gerar detalhes completos para rios, estradas e ilhas', () {
        final result = service.generateDetailedRiversRoadsIslands();
      ...
      test('deve gerar detalhes completos para castelos e fortes', () {
        ...
        expect(result.size, isNotEmpty);
        expect(result.defenses, isNotEmpty);
        expect(result.occupants, isNotEmpty);
      });

      test('deve gerar detalhes completos para templos e santuários', () {
        final result = service.generateDetailedTempleSanctuary();

        expect(result, isNotNull);
        expect(result.type, isA<TempleSanctuaryType>());
        expect(result.description, isNotEmpty);
        expect(result.details, isNotEmpty);
        expect(result.deity, isNotEmpty);
        expect(result.occupants, isNotEmpty);
      });

      test('deve gerar detalhes completos para perigos naturais', () {
        final result = service.generateDetailedNaturalDanger();
      ...
      test('deve gerar detalhes completos para civilização', () {
      ...
      test('deve separar informações em seções distintas', () {
        final result = service.generateDetailedAncestralDiscovery();

        // Verifica se as informações estão separadas em seções
        expect(result.description, contains('Tipo:'));
        expect(result.description, contains('Condição:'));
        expect(result.description, contains('Material:'));
        expect(result.description, contains('Estado:'));
        expect(result.description, contains('Guardião:'));
      });

      test('deve incluir rolagens de dados nas descrições', () {
        final result = service.generateDetailedRuin();

        // Verifica se as rolagens estão incluídas
        expect(result.description, contains('1d6'));
        expect(result.details, contains('Tabela 4.5'));
      });

      test('deve gerar resultados diferentes em múltiplas execuções', () {
        ...
        // Deve ter pelo menos algumas variações
