...
void main() {
  group('Lair Automation Tests', () {
    ...
    group('Dungeon Generation Automation', () {
      test(
        'should generate dungeon automatically with resolved dice rolls',
        () {
    ...
    group('Dice Roll Resolution Test', () {
      test('should resolve all dice rolls in lair descriptions', () {
        ...
        // Verificar se não há strings de rolagem não resolvidas (mas permitir palavras com 'd')
        expect(lair.description, isNot(matches(RegExp(r'\d+d\d+'))));
        expect(lair.occupant, isNot(matches(RegExp(r'\d+d\d+'))));

        if (lair.type == LairType.dungeons) {
          final dungeon = service.generateDungeon();
          expect(dungeon.description, isNot(matches(RegExp(r'\d+d\d+'))));
        }
