...
void main() {
  group('TreasureResolver', () {
    group('resolve', () {
      ...
      test('should not include standalone numbers without units in complex formulas', () {
        // Testa fórmulas complexas que podem ter números soltos
        final result1 = TreasureResolver.resolve('1d6 x 100 PP + 1d6 x 10 PO + 1d4 Gemas');
        final result2 = TreasureResolver.resolve('1d10 x 100 PP + 1d6 x 100 PO + 1d4 Gemas');
        final result3 = TreasureResolver.resolve('1d6 x 1.000 PP + 1d6 x 200 PO + 1d6 Gemas + 1d4 Objetos de Valor');
        
        // Verifica se não há números soltos sem unidade
        expect(result1, isNot(contains(RegExp(r'\b\d+\s*\+')))); // Não deve ter "número +"
        expect(result2, isNot(contains(RegExp(r'\b\d+\s*\+')))); // Não deve ter "número +"
        expect(result3, isNot(contains(RegExp(r'\b\d+\s*\+')))); // Não deve ter "número +"
        
        // Verifica se o resultado contém as unidades esperadas
        expect(result1, contains('PP'));
        expect(result1, contains('PO'));
        expect(result1, contains('Gemas'));
        
        expect(result2, contains('PP'));
        expect(result2, contains('PO'));
        expect(result2, contains('Gemas'));
        
        expect(result3, contains('PP'));
        expect(result3, contains('PO'));
        expect(result3, contains('Gemas'));
        expect(result3, contains('Objetos de Valor'));
      });
    });
  });
}
