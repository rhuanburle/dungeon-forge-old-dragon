    ...
    group('resolve', () {
      test('should resolve simple treasure formulas', () {
        ...
        for (final formula in testCases) {
          ...
          expect(
            result,
            isNot(equals(formula)),
          ); // Should be resolved, not the same
        }
      });
      ...
      test('should handle "Jogue Novamente" formulas', () {
        ...
        for (final formula in testCases) {
          ...
          expect(
            result,
            contains('Item Mágico'),
          ); // Should contain resolved magic item
        }
      });
      ...
      test('should handle magic item types', () {
        ...
        for (final type in magicItemTypes) {
          ...
          expect(
            result,
            isNot(equals(type)),
          ); // Should be resolved to specific item
        }
      });

      test('should return input unchanged for non-formula strings', () {
        final nonFormulas = ['Item Mágico específico', 'Tesouro encontrado'];
      ...
      test('should resolve gemas correctly', () {
        ...
        for (int i = 0; i < 10; i++) {
          ...
          final hasValidGemCategory =
              result.contains('Preciosa') ||
      ...
      test('should handle edge cases with numbers correctly', () {
        ...
        final result3 = TreasureResolver.resolve(
          '1d6 x 100 PP + 5 + 1d4 Gemas',
        );
      ...
      test(
        'should not include standalone numbers without units in complex formulas',
        () {
          // Testa fórmulas complexas que podem ter números soltos
          final result1 = TreasureResolver.resolve(
            '1d6 x 100 PP + 1d6 x 10 PO + 1d4 Gemas',
          );
          final result2 = TreasureResolver.resolve(
            '1d10 x 100 PP + 1d6 x 100 PO + 1d4 Gemas',
          );
          final result3 = TreasureResolver.resolve(
            '1d6 x 1.000 PP + 1d6 x 200 PO + 1d6 Gemas + 1d4 Objetos de Valor',
