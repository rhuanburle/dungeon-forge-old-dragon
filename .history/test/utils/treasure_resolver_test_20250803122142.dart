...
void main() {
  ...
  group('TreasureResolver By Level Tests', () {
    ...
    test('should apply level 1 treasure correctly', () {
      final treasure = TreasureResolver.resolveByLevel(TreasureLevel.level1);

      // Verifica se o tesouro do nível 1 tem os componentes corretos
      expect(treasure, contains('PP:'));
      expect(treasure, contains('Gemas:'));
      expect(treasure, contains('Objetos de Valor:'));
      expect(treasure, contains('Itens Mágicos:'));

      // Verifica se tem "Nenhum Item Mágico" (nível 1 não tem itens mágicos)
      expect(treasure, contains('Nenhum Item Mágico'));

      // Verifica se tem pelo menos PP (nível 1 sempre tem PP)
      expect(treasure, contains('PP:'));
    });

    test('should not have magic items in level 1 treasure', () {
      final treasure = TreasureResolver.resolveByLevel(TreasureLevel.level1);

      // Verifica se NÃO contém itens mágicos reais (apenas "Nenhum Item Mágico")
      expect(treasure, contains('Nenhum Item Mágico'));
      expect(treasure, isNot(contains('Poção')));
      expect(treasure, isNot(contains('Pergaminho')));
      expect(treasure, isNot(contains('Anel')));
      expect(treasure, isNot(contains('Haste')));
      expect(treasure, isNot(contains('Sacola')));
    });

    test('should extract treasure components correctly', () {
      // Testa com nível 4-5 que tem todos os componentes
      final treasure = TreasureResolver.resolveByLevel(TreasureLevel.level4to5);

      // Simula a extração de componentes
      final lines = treasure.split(' + ');
      String treasureComponent = '';
      String specialTreasureComponent = '';
      String magicItemComponent = '';

      for (final line in lines) {
        if (line.contains('PO:') || line.contains('PP:')) {
          treasureComponent +=
              (treasureComponent.isNotEmpty ? ' + ' : '') + line;
        } else if (line.contains('Gemas:') ||
            line.contains('Objetos de Valor:')) {
          if (!line.contains('Nenhuma Gema') &&
              !line.contains('Nenhum Objeto de Valor')) {
            specialTreasureComponent +=
                (specialTreasureComponent.isNotEmpty ? ' + ' : '') + line;
          }
        } else if (line.contains('Itens Mágicos:')) {
          if (!line.contains('Nenhum Item Mágico')) {
            magicItemComponent +=
                (magicItemComponent.isNotEmpty ? ' + ' : '') + line;
          }
        }
      }

      // Verifica se os componentes foram separados corretamente
      expect(treasureComponent, contains('PO:'));
      expect(treasureComponent, contains('PP:'));

      // Verifica se specialTreasure e magicItem podem estar vazios ou preenchidos
      // (depende do resultado aleatório)
      expect(treasureComponent, isNotEmpty);
    });

    test('should extract quantity only from gemas and objetos', () {
      // Testa a extração de quantidade
      final testCases = [
        {
          'input': '8 Gemas: Decorativa (10 PO), Semipreciosa (100 PO)',
          'expected': '8 Gemas',
        },
        {
          'input': '5 Objetos de Valor: Objetos de Marfim, Sacas de Especiaria',
          'expected': '5 Objetos de Valor',
        },
        {'input': '3 Gemas: Preciosa (500 PO)', 'expected': '3 Gemas'},
        {'input': '1 Objeto de Valor: Louças', 'expected': '1 Objeto de Valor'},
      ];
      ...
      for (final testCase in testCases) {
        ...
        // Simula a extração usando regex
        ...
        if (match != null) {
          ...
          final result = '$quantity $type';
          expect(result, equals(expected));
        }
      }
    });

    test('should group identical gemas and objetos', () {
      // Testa se gemas iguais são agrupadas usando resolveByLevel
      final gemasResult = TreasureResolver.resolveByLevel(TreasureLevel.level4to5);
      
      // Verifica se contém agrupamento (ex: "2 Decorativa (10 PO)")
      if (gemasResult.contains('Gemas:')) {
        expect(gemasResult, contains('Gemas:'));
      }
      
      // Testa se objetos iguais são agrupados
      final objetosResult = TreasureResolver.resolveByLevel(TreasureLevel.level6to7);
      
      // Verifica se contém agrupamento
      if (objetosResult.contains('Objetos de Valor:')) {
        expect(objetosResult, contains('Objetos de Valor:'));
      }
    });
  });
}
