    ...
    group('Tabela 4.39 - Civilização', () {
      test('Tipo, população e cálculos aleatórios', () {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          results[result.type.toString()] =
              (results[result.type.toString()] ?? 0) + 1;
        }
    ...
    group('Tabela 4.41 - Detalhamento de Povoados', () {
      test('Detalhes completos de povoados', () {
        ...
        for (int i = 0; i < 50; i++) {
          ...
          expect(aparencias.any((ap) => result.details.contains(ap)), isTrue);
          expect(alinhamentos.any((al) => result.details.contains(al)), isTrue);
          expect(
            governantes.any((gov) => result.details.contains(gov)),
            isTrue,
          );
          expect(racas.any((r) => result.details.contains(r)), isTrue);
        }
      });
    });
    ...
    group('Tabela 4.42 - Atitude e Temas', () {
      test('Atitude e temas de povoados', () {
        ...
        for (int i = 0; i < 50; i++) {
          final result = service.generateDetailedCivilization();
          expect(result, isNotNull);
          expect(atitudes.any((a) => result.details.contains(a)), isTrue);
          expect(
