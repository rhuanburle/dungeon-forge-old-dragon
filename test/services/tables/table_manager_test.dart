...
void main() {
  group('TableManager', () {
    ...
    test('should find table by name', () {
      final dungeonTable =
          manager.getTableByName('Tabela 9.1 - Geração de Masmorras');
      expect(dungeonTable, isNotNull);
      expect(dungeonTable!.tableName, 'Tabela 9.1 - Geração de Masmorras');

      final subterraneanTable =
          manager.getTableByName('Tabela A13.1 - Encontros no Subterrâneo');
      expect(subterraneanTable, isNotNull);
      expect(subterraneanTable!.tableName,
    ...
    test('should check if table exists', () {
      ...
      expect(
          manager.hasTable('Tabela A13.1 - Encontros no Subterrâneo'), isTrue);
    ...
    test('should work with monster encounter tables', () {
      ...
      expect(subterraneanTable.getByPartyLevel(PartyLevel.beginners, 2),
          MonsterType.giantRat);
      expect(plainsTable.getByPartyLevel(PartyLevel.advanced, 2),
          MonsterType.troll);
    });
  });
}
